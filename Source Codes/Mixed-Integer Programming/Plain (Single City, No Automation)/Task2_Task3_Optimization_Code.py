...
def Output(m, city_name, district):  
    # Print the result:
    status_code = {1:'LOADED', 2:'OPTIMAL', 3:'INFEASIBLE', 4:'INF_OR_UNBD', 
                   5:'UNBOUNDED'}
    status = m.status
    
    file_name = city_name + "_" + district + "_Belediye_Sonuc.txt"
    text_file = open(file_name, "w")
    text_file.write(city_name + "-" + district + " Belediyesi Seçilen Mahalleler\n\n")
...
def OptNeighborhoods(m_2014, m_2019, n, k, G_2014, G_2019, M_2014, M_2019, neigh_names, city_name, district):
    # Create the model
    ...
    Output(model, city_name, district)
