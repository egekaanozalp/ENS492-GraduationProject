...
def Output(m, city_name, district, output_dir):  
    # Print the result:
    status_code = {1:'LOADED', 2:'OPTIMAL', 3:'INFEASIBLE', 4:'INF_OR_UNBD', 
                   5:'UNBOUNDED'}
    status = m.status
    
    file_name = os.path.join(output_dir, city_name + "_" + district + "_Belediye_Meclisi_Sonuc.txt")
    with open(file_name, "w") as text_file:
        text_file.write(city_name + " " + district + " Belediye Meclisi Seçilen Mahalleler\n\n")
...
def OptNeighborhoods(m_2014, m_2019, n, k, G_2014, G_2019, M_2014, M_2019, neigh_names, city_name, district, output_dir):
    # Create the model
    ...
    Output(model, city_name, district, output_dir)
    
def main():
    
    # Folder paths
    base_path = "C:/Users/Pc/Desktop/Dersler/4.2/ENS492/Son deneme/Belediye_Meclis/"
    ...
    for city_name in city_files:
        ...
        # Find the unique district names:
        district_names = df_election_2014["İlçe Adı"].unique()
        ...
        for district in district_names:
            ...
            M_2014 = df_election_2014[df_election_2014["İlçe Adı"] == district][columns_2014].fillna(0)
            M_2019 = df_election_2019[df_election_2019["İlçe Adı"] == district][columns_2019].fillna(0)
            M_2014_2 = df_election_2014_2[df_election_2014_2["İlçe Adı"] == district][columns_2014].fillna(0)
            M_2019_2 = df_election_2019_2[df_election_2019_2["İlçe Adı"] == district][columns_2019].fillna(0)
            
            # Adjust the neighbor names:
            neigh_names = [row["Mahalle/Köy"].replace(" ", "_").replace("İ", "I")
                           .replace("Ö", "O").replace("Ğ", "G").replace("Ü", "U")
                           .replace("Ş", "S").replace("Ç", "C") 
                           for index, row in df_election_2014
                           [df_election_2014["İlçe Adı"] == district].iterrows()]
            
            
            # Actual result of the parties:
            if M_2014_2.sum().sum() != 0:
                G_2014 = [(M_2014_2.sum()[i])/(M_2014_2.sum().sum()) for i in range(len(M_2014_2.sum()))]
            else:
                G_2014 = [0 for i in range(len(M_2014_2.sum()))]
                
            if M_2019_2.sum().sum() != 0:
                G_2019 = [(M_2019_2.sum()[i])/(M_2019_2.sum().sum()) for i in range(len(M_2019_2.sum()))]
            else:
                G_2019 = [0 for i in range(len(M_2019_2.sum()))]
